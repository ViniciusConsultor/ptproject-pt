...
namespace IN202
{
    public partial class IN202 : PT.Master.GridEditIn
    {
        ...
        string _strFistChar = "CK", _strLastNbr;
        ...
        {
            if (txtQty.Text.Trim() != "")
                _fltQty = float.Parse(txtQty.Number.ToString());
            else
                _fltQty = 0;

            if (txtUnitCost.Text.Trim() != "")
                _fltUnitCost = float.Parse(txtUnitCost.Number.ToString());
            else
                _fltUnitCost = 0;
        ...
        {
            if (txtQty.Text.Trim() != "")
                txtUnitCost.Text = (float.Parse(txtTotalCost.Number.ToString()) / float.Parse(txtQty.Number.ToString())).ToString();
            //if (txtUnitCost.Text.Trim() != "")
        ...
        private void txtProductID_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtProductID.Text.Trim() != "")
            {
                _dtProductInfo = ConnectDB.ExecuteReader("sp_INProductInfoAndStatusGet", new String[] { "UserID", "ProductID" }, new Object[] { _strUser, txtProductID.Text });
        ...
        //    if (cmbType.SelectedValue != null)
        //    {
            DataTable _dtLastNbr = IN202Ctrl.GetSASetupGetLastNbr("IN", _strFistChar);
        ...
        {
            _dtProductList = ConnectDB.ExecuteReader("sp_INProductGetAll");
        ...
        private void _BindDocList()
        {
            ...
            dgv.DataSource = IN202Ctrl.GetINDocListByPara("IN202", _strUser, strTranType, dtmFromDate.Value.Date, dtmToDate.Value.Date);
            dgv.ReadOnly = true;
        ...
        {
            _dtDocDet = IN202Ctrl.GetINDocDetByDoc(BranchID, DocNbr, "IN");
        ...
        private void _DeleteDocDet(int row)
        {
            ...
            IN202Ctrl.DeleteINDocDet(strBranhID, strDocNbr, strModule, strLineRef);
        }
        ...
        private void _AddRowToDocDetGrid()
        {
            ...
            _r["LineQty"] = float.Parse(txtQty.Number.ToString());
            _r["UnitCost"] = float.Parse(txtUnitCost.Number.ToString());
            _r["TotalCost"] = float.Parse(txtTotalCost.Number.ToString());
            _r["InvtMult"] = -1;            
            _r["Cnvfact"] = _fltCnvFact;
            _r["UnitRate"] = _fltUnitRate;
            _r["DetailQty"] = float.Parse(txtQty.Number.ToString()) * _fltUnitRate;
        ...
        private void _UpdateRowDocDetGrid()
        {
            ...
            if (dgvDocDet.RowCount > 0 && txtProductID.Text.Trim() != "" && txtQty.Text.Trim() != "")
            {
                ...
                _dtDocDet.Rows[_rUpd]["LineQty"] = float.Parse(txtQty.Number.ToString());
                _dtDocDet.Rows[_rUpd]["UnitCost"] = float.Parse(txtUnitCost.Number.ToString());
                _dtDocDet.Rows[_rUpd]["TotalCost"] = float.Parse(txtTotalCost.Number.ToString());
                _dtDocDet.Rows[_rUpd]["InvtMult"] = -1;                
                _dtDocDet.Rows[_rUpd]["Cnvfact"] = _fltCnvFact;
                _dtDocDet.Rows[_rUpd]["UnitRate"] = _fltUnitRate;
                _dtDocDet.Rows[_rUpd]["DetailQty"] = float.Parse(txtQty.Number.ToString()) * _fltUnitRate;
        ...
        private void _BinEditPanel(string BranchID, string DocNbr, string Module)
        {
            ...
            _dtDocEdit = IN202Ctrl.GetINDocByKey(BranchID, DocNbr, "IN");
            ...
            cmbWhID.SelectedItem = _dtDocEdit.Rows[0]["ToWhID"].ToString();
            ...
            label1.Text = _dtDocEdit.Rows[0]["Version"].ToString();
